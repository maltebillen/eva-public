#ifndef EVA_SEGMENT_CENTRALISED_NETWORK_NODES_H
#define EVA_SEGMENT_CENTRALISED_NETWORK_NODES_H
...
namespace eva
{
	namespace sbn
	{
		namespace cen
		{
            enum class CenNodeType : uint8_t
			{
				SEGMENT = 1,
				START_SCHEDULE,
                CENTRALISED_CHARGING,
				FIXED_CHARGING,
				END_SCHEDULE,
				UNDEFINED
			};

            class CenBaseNodeData
			{
			public:
				// INLINE FUNCTIONS
				inline std::unique_ptr<CenBaseNodeData> clone() const { return std::unique_ptr<CenBaseNodeData>(deepClone()); };
				...
				virtual ~CenBaseNodeData() {};

			protected:
				virtual inline CenBaseNodeData *deepClone() const = 0;
			};

			class UndefinedNodeData : public CenBaseNodeData
			{
			public:
				// CONSTRUCTORS
				UndefinedNodeData() {};
			...
			class SegmentPieceNodeData : public CenBaseNodeData
			{
				// ATTRIBUTES
            ...
            class CentralisedChargingNodeData : public CenBaseNodeData
			{
			...
			public:
				...
				CentralisedChargingNodeData(
					const Charger &charger) : _indexCharger(charger.get_index())
				{
				}

				CentralisedChargingNodeData(
					const Types::Index &indexCharger) : _indexCharger(indexCharger)
				{
				}
			...
			protected:
				virtual inline CentralisedChargingNodeData *deepClone() const override { return new CentralisedChargingNodeData(*this); };
			};

			class StartScheduleNodeData : public CenBaseNodeData
			{
				const Types::Index _indexCharger;
			...
			class FixedChargingNodeData : public CenBaseNodeData
			{
			...
			class EndScheduleNodeData : public CenBaseNodeData
			{
			...
			public:
				std::unique_ptr<CenBaseNodeData> ptrNodeData;
				Types::Index index = Constants::BIG_INDEX;
				CenNodeType type = CenNodeType::UNDEFINED;

				// CONSTRUCTORS

				CenNodeData() : index(Constants::BIG_INDEX),
										 type(CenNodeType::UNDEFINED),
										 ptrNodeData(std::make_unique<UndefinedNodeData>()) {};

				CenNodeData(
					const SegmentPieceNodeData &spnd,
					const uint32_t &nrVehicles) : type(CenNodeType::SEGMENT),
				...
				CenNodeData(
					...
					const uint32_t &nrVehicles) : type(CenNodeType::START_SCHEDULE),
                ...
                CenNodeData(
					const CentralisedChargingNodeData &ccnd,
					const uint32_t &nrVehicles) : type(CenNodeType::CENTRALISED_CHARGING),
												  ptrNodeData(std::make_unique<CentralisedChargingNodeData>(ccnd))
				{
					_vecAccess.resize(nrVehicles, Types::AccessType::ALLOWED);
				};
				...
				CenNodeData(
					...
					const uint32_t &nrVehicles) : type(CenNodeType::FIXED_CHARGING),
				...
				CenNodeData(
					...
					const uint32_t &nrVehicles) : type(CenNodeType::END_SCHEDULE),
				...
				CenNodeData(CenNodeData const &other) : index(other.index),
																		  type(other.type),
				...
				CenNodeData(CenNodeData &&other) = default;
				CenNodeData &operator=(CenNodeData const &other)
				{
					ptrNodeData = other.ptrNodeData->clone();
					index = other.index;
					type = other.type;
				...
				CenNodeData &operator=(CenNodeData &&other) = default;
				...
				inline const CenNodeType get_type() const { return type; };
...
#endif // !EVA_SEGMENT_CENTRALISED_NETWORK_NODES_H